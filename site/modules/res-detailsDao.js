...
async function getAllReviewsForRes(business_id) {
    const db = await database;
    const reviews = await db.query( "select a.name, a.user_id,b.stars, b.text, b.business_id, b.date from users as a, reviews as b where a.user_id=b.user_id and b.business_id="+business_id+" ORDER BY b.date DESC");
    return reviews;
}
